...
namespace ItManagement.Folder
{
        ...
        where T : class
    {
        ...
        {
            return await InvokeAPIWithReturnValueAsync<List<T>>(() =>
                _httpClient.GetAsync(BuildRequestURI(APIMethod.Load)));
        }

        public async Task<T> Read(int key)
        {
            return await InvokeAPIWithReturnValueAsync<T>(() =>
                _httpClient.GetAsync(BuildRequestURI(APIMethod.Read, key)));
        }

        public async Task Create(T obj)
        {
            await InvokeAPINoReturnValueAsync(() =>
                _httpClient.PostAsJsonAsync(BuildRequestURI(APIMethod.Create), obj));
        }

        public async Task Update(int key, T obj)
        {
            await InvokeAPINoReturnValueAsync(() =>
                _httpClient.PutAsJsonAsync(BuildRequestURI(APIMethod.Update, key), obj));
        }
